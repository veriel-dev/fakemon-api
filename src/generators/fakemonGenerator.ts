...
function generateName(type: string): string {
  ...
  const prefix =
    components.prefixes[Math.floor(Math.random() * components.prefixes.length)];
  const suffix =
    components.suffixes[Math.floor(Math.random() * components.suffixes.length)];
...
export function generateFakemon(type: string): Partial<Fakemon> {
  ...
  const { normalAbility, hiddenAbility } = generateAbilities(type);
  return {
    name,
    stats,
